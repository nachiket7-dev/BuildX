...
function getFileIcon(path: string) {
  ...
  switch (ext) {
    ...
    case 'js':
    case 'jsx':
      return <FileCode size={13} className="text-purple-400 shrink-0" />;
    case 'json':
      return <FileJson size={13} className="text-amber-400 shrink-0" />;
    case 'sql':
    case 'prisma':
      return <Database size={13} className="text-emerald-400 shrink-0" />;
    case 'md':
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-2 font-mono-custom text-xs">
        ...
        <div className="space-y-2 pt-1">
          {Array.from({ length: 7 }).map((_, i) => (
            <div
              key={i}
              className="flex items-center gap-2.5 px-2.5 py-1.5 rounded-lg bg-white/5 animate-pulse"
            >
