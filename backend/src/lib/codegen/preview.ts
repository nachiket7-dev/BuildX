...
function renderStorefrontDiscovery(accent: ReturnType<typeof pickAccent>): string {
  const restaurants = [
...
function renderAuth(screen: UiScreen, appName: string): string {
...
export function buildDeterministicPreview(blueprint: Blueprint): string {
  ...
  const screenContents = navItems.map(item => {
    let content = '';
    switch (item.kind) {
      case 'storefront': content = renderStorefrontDiscovery(accent); break;
      ...
      case 'auth':       content = renderAuth(item.screen, blueprint.appName); break;
